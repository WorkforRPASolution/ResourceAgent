...
| **60s** | disk, temperature, fan, gpu, cpu_process, memory_process, voltage, motherboard_temp, process_watch | 9 | 9회 (1×9) |
| **300s** | storage_smart, storage_health, uptime | 3 | 0.6회 |
| | | **합계 15** | **~16회/분** |
...
├─────────────────────────────────────────────────────────────────┤
│  WMI/smartctl 기반 (LhmHelper 불필요)                             │
│  └─ storage_health (300s)           Windows: WMI, Linux: smartctl│
└─────────────────────────────────────────────────────────────────┘
```
...
```
Start() → 15개 고루틴 생성 (collector당 1개)
...
분당 ~16개 MetricData 전송 (gpu=0 rows 시 실질 ~15개):
