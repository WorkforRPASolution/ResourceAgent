...
| 단계 | 명령 | 효과 |
|---|---|---|
| 1 | `sc failure ... restart/5000/10000/30000` | 복구 액션(재시작 5s→10s→30s, 이후 30s 반복) 보장 |
...
REM 시작 유형을 자동(지연)으로 바꾸지 않고 기존 start=auto 유지
...
| `/noauto` | 시작 유형 변경(3단계) 생략 — 기존 `start=auto` 유지 |
...
[1/4] Registering failure actions (restart 5s/10s/30s)...
  OK
...
> 복구 액션 자체(restart 5/10/30s)는 그대로 두어도 무방합니다(크래시 시에만 추가로 동작).
...
- **재설치 시:** 업데이트된 `install_ResourceAgent.bat`은 이제 **`failureflag=1`을 기본 포함**하므로 재설치해도 자가복구(복구 정책 발동)는 유지됩니다. 다만 시작 유형은 `start= auto`로 설정되어(지연 시작 아님) 부팅 경쟁 완화 효과는 사라지므로, **지연 시작까지 원하면** 재설치 후 이 핫픽스를 다시 실행하세요. (구버전 설치본으로 설치된 PC는 `failureflag`가 없으므로 이 핫픽스 적용이 필요합니다.)
