...
- `scripts/install_ResourceAgent.bat`:
  ...
  - 미설치 시 번들된 설치기 자동 실행 (`/passive /norestart`)
  - 재부팅 필요 시(`errorlevel 3010/1641`) 서비스 등록만 하고 자동 시작 스킵 (재부팅 후 start=auto로 자동 실행)
  - errorlevel 정확한 비교(`%errorlevel% EQU N`) 사용 (`if errorlevel N`은 "N 이상" 의미라 부정확)
  - 설치 결과를 `%LOG_DIR%\install_result.txt`에 기록 (ResourceAgent가 Kafka로 보고 예정)
